...
df_randsample = df_merged.sample(n=500) # taking n random datapoints as a sample 
...
list(df_sample['Freq']) # to print the entire dataframe column
...
threshold = 0.90 # percent of occurence of string in each BOLType
...
## removing the common elements in a list of lists
#a = df_sample_test['Freq'] # list of lists
#b_set = set(tuple(x) for x in a) # change to a set to remove duplicates
#b = [ list(x) for x in b_set ]
#b.sort(key = lambda x: a.index(x)) # sorting the list
#
#type(df_sample_test['Freq'][0]) #list
#
## Another method
#b = list()
#for sublist in a:
...
#
## Another method
#for i in range(len(df_sample_test['Freq'])-1):
#    [elem for elem in df_sample_test['Freq'][:] if elem not in df_sample_test['Freq'][i]]
#
#print((list(df_sample_test['Freq'])))
#type(df_sample_test['Freq'][0])
#

# --------------------------------

# Checking for elements which are unique across all BOLs+
# Convert the entire column 'Freq' to a LIST

# Convert list to np.array
#from numpy import array
#for i in range(len(df_sample_test['Freq'])):
#    df_sample_test['Freq'][i] = array(df_sample_test['Freq'][i])
#
#master = []
#for sublist in df_sample_test['Freq']:
#    for item in sublist:
#        master.append(item)
#
#type(master)
#master = array(master)
#master = np.unique(master) #removing non-unique elements
#
### Using np.array - combine all lists with unique elements
##merged_list = np.concatenate(df_sample_test['Freq'].values)
##unique, counts = np.unique(merged_list, return_counts=True)
##master = unique[counts==1] # only include elements which occur once ie. count = 1
##master
...
#    concat_arr = np.concatenate(df_sample_test[df_sample_test['BOLType'] == key]['Freq'].values)
...
#    BOL_identifier = sorted(dict2.keys()) # sorting the column by values
#    type(BOL_identifier) #list
#    print('\n')
#    print("BOLType:", key)
#    print("BOL Identifier Keywords:", BOL_identifier, '\n')
#
...
## ------------------------------------------------------------------------------------
#
## Numpy Method - To calculate the frequency of unique elements in the same BOL Type
#
## Convert the entire column 'Freq' to a LIST
#import numpy as np
#for i in range(len(df_sample)):
...
## Checking for elements which are unique across all BOLs+
## Using np.array - combine all lists with unique elements
#merged_list = np.concatenate(df_sample['Freq'].values)
#unique, counts = np.unique(merged_list, return_counts=True)
#master = unique[counts==1] # only include elements which occur once ie. count = 1
#master
#
## taking the intersection of each list with the merged list (keeping only the elements which are unique)
#for i in range(len(df_sample['Freq'])):
#    df_sample['Freq'][i] = np.intersect1d(df_sample['Freq'][i], master)
#df_sample['Freq']
#
#freq = df_sample['BOLType'].value_counts().to_dict() # Converting BOL Type column into a dictionary
#type(freq)
...
#
## ---------------------------------------------------------------------------------------
# Plot for observing frequency of each word 
#corpus = master
